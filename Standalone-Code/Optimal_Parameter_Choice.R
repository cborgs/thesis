#########################################################################
#
# Estimating optimal parameter choices for an input file specified
# 
...
# Packages required: PPRL, fastdigest, stringdist, entropy, data.table, fastLink, moments, ineq, multibitTree
# User-defined settings in lines 28 to 58.
#
# This script reads the file(s), generates all measures required for the estimation, 
# fits a model and predicts the optimal parameters.
# Optionally, includes possibility to generate multiple iterations, e.g. for bootstrapped estimates.
# Gives optimal estimates for a simpe linear model, a model generated by BIC- or AIC-based model selection
# and a response surface model estimation (RSM).
#
# Output: 
#    Optimal_Parameters_LM.csv 
#    Optimal_Parameters_BIC.csv
#    Optimal_Parameters_RSM.csv
#    Optimal_Parameters_RF.csv
#
#
...
BIC <- stepAIC(fullmod, minmod, direction = "both",  k = log(nrow(fixed))) # bic
...
# RSM: Preproc data
RSMdata <- fixed[,c("k", "q", "u", "meanengrams", "hammingweight", "Fmeasure")]
RSMdata_unscaled <- fixed[,c("k", "q", "u", "meanengrams", "hammingweight", "Fmeasure")]
# Scale min/max
...
save(ests, file="EM_ests.Rdata") # In case of or if pre-estimated file exists
load("EM_ests.Rdata")
...
for (q in Q){
    ...
    for (k in K){
      ...
      for (i in seq(1:bootstraps)){
        ...
        # Append to result
        meta  <- rbind(meta, data.frame(d="New Data", type, q, k, i, l, Tani=tanimotoGoal,Fmeasure, filesizeA, filesizeB, meanentropy, meanengrams, sdgrams, skew, ginicoefficient, ginicoefficientqgrams, errorestimation , missingamount,hammingweight, uniqueness, sduniqueness, meanmissing, missingamount, uniquepatternsA,uniquepatternsB,m,u))
...
write.table(parameters, "Optimal_Parameters_LM.csv",sep="\t", row.names = FALSE)
...
write.table(parametersRSM, "Optimal_Parameters_RSM.csv",sep="\t", row.names = FALSE)

cat("\n All done!")

cat("\n Best parameters (k/q):\n\n Simple linear model:", parameters$k, "/", parameters$q, "\n BIC-selected linear model:", 
    parameters2$k, "/", parameters2$q,"\n Random Forest model:",  parameters3$k, "/", parameters3$q,"\n Response Surface Method:",  parametersRSM$k, "/", parametersRSM$q, "\n")
