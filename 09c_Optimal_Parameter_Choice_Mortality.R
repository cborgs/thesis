#########################################################################
#
# Estimating optimal parameter choices for Mortality data
#
#########################################################################
...
Stringidents <- c("Vorname","Nachname")
...
for (q in Q){
    ...
    for (k in K){
      ...
      for (i in seq(1:bootstraps)){
        ...
        # Append to result
        meta  <- rbind(meta, data.frame(d="Mortality Test Data", type, q, k, i, l, Tani=tanimotoGoal,Fmeasure, filesizeA, filesizeB, meanentropy, meanengrams, sdgrams, skew, ginicoefficient, ginicoefficientqgrams, errorestimation , hammingweight, uniqueness, sduniqueness, meanmissing, missingamount, uniquepatternsA,uniquepatternsB,m,u))
