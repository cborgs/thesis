#########################################################################
#
# Estimating optimal parameter choices for NC Voter data
#
#########################################################################
...
inputFile <- "Daten/ncvoter-20140619-temporal-balanced-ratio-1to1-a.csv"
# Identifiers
idents <- c("first_name","last_name","Year")
...
Stringidents <- c("first_name","last_name")
...
inputFileB <- "Daten/ncvoter-20140619-temporal-balanced-ratio-1to1-b.csv"
...
if (mode == "Link"){
  ...
  # Draw sample
  set.seed(42)
  clearTextA <- clearTextA[sample(1:nrow(clearTextA), 50000, replace=FALSE),]
  set.seed(42)
  clearTextB <- clearTextB[sample(1:nrow(clearTextB), 50000, replace=FALSE),]
  
  
  clearTextA$Day <- ""
  clearTextA$Month <- ""
  clearTextA$Year <- as.character(2014-as.numeric(clearTextA$age))
  
  clearTextB$Day <- ""
  clearTextB$Month <- ""
  clearTextB$Year <- as.character(2014-as.numeric(clearTextB$age))
  clearTextA$age <- NULL
  clearTextB$age <- NULL
...
## Deduplicate by ID
clearTextA <- clearTextA[!duplicated(clearTextA[,..IDcol]),]
clearTextB <- clearTextB[!duplicated(clearTextB[,..IDcol]),]
...
for (q in Q){
    ...
    for (k in K){
      ...
      result <- multibitTree.searchFile("TreeA_OPC.csv", Tani)
        ...
        # Append to result
        meta  <- rbind(meta, data.frame(d="NC Voter", type, q, k, i, l, Tani=tanimotoGoal,Fmeasure, filesizeA, filesizeB, meanentropy, meanengrams, sdgrams, skew, ginicoefficient, ginicoefficientqgrams, errorestimation , hammingweight, uniqueness, sduniqueness, meanmissing, missingamount, uniquepatternsA,uniquepatternsB,m,u))
...
write.table(newdata, "Trainresults_NCVoter.csv", sep="\t", row.names=FALSE, quote=FALSE, col.names=TRUE)
newdata <- read.csv("Trainresults_NCVoter.csv", sep = "\t", stringsAsFactors = FALSE)
