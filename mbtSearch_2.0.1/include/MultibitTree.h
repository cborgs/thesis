...
class MultibitTree {
	...
	int64_t mLeafStart;		// start of tree in the array of Fingerprints
	int64_t mSize;			// length of tree in the array of Fingerprints
	...
	P **mLeaves;			// pointer to an array of Fingerprints
	
	FingerprintSplitter<P> mSplitter;	// helper class for splitting and match bit computation
	...
	void internalSearch(QueryResult*, FingerprintTanimoto*, int64_t, int, int, int, int, S);
	// recursively search sub tree by hamming distance
	void internalSearch(QueryResult*, FingerprintHamming*, int64_t, int, int, int, S);

	public:

	// constructor
	// create a new MultibitTree from an array of Fingerprints
	MultibitTree(P **prints, int64_t leafStart, int64_t leafEnd, int nBits, int cardinality, int leafLimit);
...
template <class TT>
MultibitTree<TT>::MultibitTree(P **prints, int64_t leafStart, int64_t leafEnd, int nBits, int cardinality, int leafLimit) :
...
template <>
void MultibitTree<FingerprintTanimoto>::internalSearch(QueryResult *result, FingerprintTanimoto *queryPrint, int64_t node, int commonXOR, int AB, int queryUnmatched, int treeUnmatched, float minTanimoto) {	
	int size;
	ushort *matchBitIdx;
	...
	if (size & LEAF_BIT) {
		...
		for (int64_t i = mLeftChild[node]; i < mRightChild[node]; i++) {
			FingerprintTanimoto *leaf = mLeaves[i];
...
template <>
void MultibitTree<FingerprintHamming>::internalSearch(QueryResult *result, FingerprintHamming *queryPrint, int64_t node, int AB, int queryUnmatched, int treeUnmatched, int maxHamming) {
	int size;
	ushort *matchBitIdx;
	...
	if (size & LEAF_BIT) {
		// if this is a leaf-node check each leaf-fingerprint's hamming distance
		for (int64_t i = mLeftChild[node]; i < mRightChild[node]; i++) {
			FingerprintHamming *leaf = mLeaves[i];
			...
			if (queryPrint->hammingXOR(leaf) <= maxHamming) {
				...
				if (hamming <= maxHamming) {
					// add matching leaf to QueryResult
					result->add(queryPrint->getId(), leaf, (float)(mNBits - hamming) / mNBits);
				}
			}
		}
	} else {
...
template <>
inline void MultibitTree<FingerprintTanimoto>::search(QueryResult *result, FingerprintTanimoto *queryPrint, int cardinality, float minTanimotoSimilarity) {
...
template <>
inline void MultibitTree<FingerprintHamming>::search(QueryResult *result, FingerprintHamming *queryPrint, int cardinality, int maxHammingDistance) {
