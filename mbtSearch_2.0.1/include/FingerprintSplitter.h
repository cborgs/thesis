...
#ifndef FINGERPRINT_SPLITTER_H
#define FINGERPRINT_SPLITTER_H

#include "Misc.h"

// objects of template class FingerprintSplitter sort
// a range of a fingerprint array by the best dividing bit 
template <class T>
class FingerprintSplitter {
	public:

	FingerprintSplitter(T **prints, int nBits) {
		mPrints = prints;
		mNBits = nBits;
		mCount = new int64_t[nBits];
	}

	~FingerprintSplitter() { delete[] mCount; };
...
// split range by best bit
template <class T>
int64_t FingerprintSplitter<T>::splitRangeHalf(int64_t leafStart, int64_t leafEnd) {
	...
	T *leaf;		// temporary pointer for swapping Fingerprints
