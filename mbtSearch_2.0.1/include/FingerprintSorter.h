...
#ifndef FINGERPRINT_SORTER_H
#define FINGERPRINT_SORTER_H

// objects of template class FingerprintSorter take an array of fingerprints
// and sort the prints by cardinality, provide information of start and
// end positions of the cardinality clusters
template <class T>
class FingerprintSorter {
	public:

	FingerprintSorter () : mClusterPos(NULL), mClusterEnd(NULL) {}
	~FingerprintSorter() { clear(); }
...
// sort print array and save cluster information
template <class T>
void FingerprintSorter<T>::cluster(T **prints, int64_t size, int nBits) {
	clear();
