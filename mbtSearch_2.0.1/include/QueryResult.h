...
class QueryResult {
	...
	FILE *mResultFile;			// file descriptor for the result file
	...
	// constructor
	QueryResult(int sort, FILE *resultFile, const char *seperator, int64_t sizeTree);

	// destructor
	~QueryResult();
	
	// add a Fingerprint and the corresponding similarity coefficient to the query result
	void add(char *queryId, Fingerprint *print, float similarity);
